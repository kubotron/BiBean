...
void imperial_step()
{
	if (buzzer_next_iteration <= sys_tick_get())
	{
		uint8_t index = song_i / 2;
		if (song_i % 2 == 1)
		{
			buzzer_set_tone(0);
			buzzer_next_iteration = sys_tick_get() + 10;
		}
		else
		{
			buzzer_set_tone(sw_note[index]);
			buzzer_next_iteration = sys_tick_get() + sw_timing[index];
		}
...
void mario_step()
{
	if (buzzer_next_iteration <= sys_tick_get())
	{
		...
		{
			buzzer_set_tone(0);
			buzzer_next_iteration = sys_tick_get() + mario_delay[index] / 1.6;
		}
		else
		{
			buzzer_set_tone((uint32_t)74932 / (uint32_t)mario_note[index]);
			buzzer_next_iteration = sys_tick_get() + mario_timing[index] * 1.5;
		}
